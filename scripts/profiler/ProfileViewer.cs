...
namespace Server.Profiler {
	public class ProfileViewer : Gump {
		public static void Initialize() {
			Server.Commands.Register("Profiler", AccessLevel.Administrator,
									 new CommandEventHandler(OnViewProfile));
		}

		[Usage("Profiler")]
		[Description("Browse profiling information")]
		public static void OnViewProfile(CommandEventArgs e) {
			e.Mobile.SendGump(new ProfileViewer(e.Mobile));
		}
		...
		public ProfileViewer(Mobile from) : base(200, 150) {
			from.CloseGump(typeof(ProfileViewer));
		...
		public override void OnResponse(Server.Network.NetState sender, RelayInfo info) {
            if (sender == null ||
				...
				sender.Mobile.AccessLevel < AccessLevel.Administrator)
                return;

			switch (info.ButtonID) {
			case 1: /* refresh */
                sender.Mobile.SendGump(new ProfileViewer(sender.Mobile));
				break;
