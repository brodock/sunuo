	...
	public abstract class BaseHouse : BaseMulti
	{
		...
		public virtual DecayType DecayType
		{
			get
			{
				...
				if ( (acct.LastLogin + TimeSpan.FromDays( 90.0 )) < Core.Now )
				...
				for ( int i = 0; i < allHouses.Count; ++i )
				{
					...
					if ( newest == null || check.m_BuiltOn > newest.m_BuiltOn )
						newest = check;
				}
		...
		public virtual DecayLevel DecayLevel
		{
			get
			{
				if ( !CanDecay )
				{
					m_LastRefreshed = Core.Now;
					return DecayLevel.Ageless;
				}

				TimeSpan timeAfterRefresh = Core.Now - m_LastRefreshed;
		...
		public bool InRange( IPoint2D from, int range )
		{
			...
			foreach ( Rectangle2D coord in Region.Coords )
			{
				if ( from.X >= coord.X - 5 && from.Y >= coord.Y - 5 && from.X < coord.End.X + 5 && from.Y < coord.End.Y + 5 )
					return true;
			}
		...
		public ArrayList GetMobiles()
		{
			...
			foreach ( Mobile mobile in Region.Mobiles )
			{
				if ( IsInside( mobile ) )
					list.Add( mobile );
			}
		...
		public virtual bool IsCombatRestricted( Mobile m )
		{
			...
			for ( int i = 0; i < m.Aggressed.Count; ++i )
			{
				...
				if ( info.Defender.Player && info.Defender.Alive && (Core.Now - info.LastCombatTime) < HouseRegion.CombatHeatDelay && (attackerGuild == null || defenderGuild == null || defenderGuild != attackerGuild && !defenderGuild.IsEnemy( attackerGuild )) )
					return true;
			}
			...
			for ( int i = 0; i < m_LockDowns.Count; ++i )
			{
				...
				if ( item is Container && !(item is BaseBoard) )
				{
					...
					for ( int j = 0; j < children.Count; ++j )
					{
						...
						if ( child.Decays && !child.IsLockedDown && !child.IsSecure && (child.LastMoved + child.DecayTime) <= Core.Now )
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( version < 11 )
				m_LastRefreshed = Core.Now + TimeSpan.FromHours( 24 * Utility.RandomDouble() );
