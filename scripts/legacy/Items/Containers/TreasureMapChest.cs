...
namespace Server.Items
{
	public class TreasureMapChest : LockableContainer
	{
		...
		public TreasureMapChest( Mobile owner, int level, bool temporary ) : base( 0xE40 )
		{
			m_Owner = owner;
			m_Level = level;
			m_DeleteTime = Core.Now + TimeSpan.FromHours( 3.0 );
		...
		{
			return CheckLoot( from, true ) && base.CheckLift (from, item);
		}
		...
		{
			to.SendLocalizedMessage( 1048122, "", 0x8A5 ); // The chest refuses to be filled with treasure again.
		}
