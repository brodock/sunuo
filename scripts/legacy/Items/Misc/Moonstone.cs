...
namespace Server.Items
{
	...
	public class Moonstone : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( Region.Find( from.Location, from.Map ).Coords != null || Region.Find( from.Location, GetTargetMap() ).Coords != null ) // Wilderness coordinates==null
			{
				from.SendLocalizedMessage( 1005401 ); // You cannot bury the stone here.
			}
		...
		private class SettleTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				else if ( m_Count >= 10 )
				{
					...
					if ( m_Count == 16 )
					{
						if ( Region.Find( m_Location, m_Map ).Coords != null || Region.Find( m_Location, m_TargetMap ).Coords != null ) // Wilderness coordinates==null
						{
