...
{
	public class Sigil : BaseSystemController, IComparable
	{
		...
		public TimeSpan TimeUntilCorruption
		{
			get
			{
				...
				TimeSpan ts = ( m_CorruptionStart + CorruptionPeriod ) - Core.Now;
			...
			else if ( obj is BaseMonolith  )
			{
				...
				if ( obj is StrongholdMonolith )
				{
					...
					else
					{
						...
						else if ( m_GraceStart > DateTime.MinValue && (m_GraceStart + CorruptionGrace) < Core.Now )
						{
							if ( m_Corrupted != newController )
		...
		{
			Town ourTown = m_Town;
			Town theirTown = ((Sigil)obj).m_Town;

			int ourSort = ( ourTown == null ? -1 : ourTown.Definition.Sort );
			int theirSort = ( theirTown == null ? -1 : theirTown.Definition.Sort );

			return ourSort - theirSort;
		}
	}
}
