...
namespace Server.Engines.Help
{
	public class SpeechLogGump : Gump
	{
		...
		public SpeechLogGump( Mobile player, SpeechLog log ) : this( player, new ArrayList( log ) )
		{
		}

		public SpeechLogGump( Mobile player, ArrayList log ) : this( player, log, ( log.Count - 1 ) / MaxEntriesPerPage )
		{
		}

		public SpeechLogGump( Mobile player, ArrayList log, int page ) : base( 500, 30 )
		{
			m_Player = player;
			m_Log = log;
			m_Page = page;
			...
			string playerAccount = player.Account is Account ? ((Account)player.Account).Username : "???";
			...
			else
			{
				...
				{
					SpeechLogEntry entry = (SpeechLogEntry) log[i];

					Mobile m = entry.From;

					string name = m.Name;
					string account = m.Account is Account ? ((Account)m.Account).Username : "???";
