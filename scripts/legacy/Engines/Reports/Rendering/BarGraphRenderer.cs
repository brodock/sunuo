...
//					sfVLabel.FormatFlags=StringFormatFlags.DirectionVertical;
					...
					graph.DrawString(_xTitle,lblFont,brs,new RectangleF( _xOrigin, _yOrigin+_graphHeight+14.0f+(_renderMode==BarGraphRenderMode.Bars?10.0f:0.0f)+(DataPoints.Count>15?16.0f:0.0f), _graphWidth, 20.0f ), lblFormat );
				}
					...
					using(SolidBrush barBrush = new SolidBrush(item.ItemColor))
					{
						using ( Pen pen = new Pen(_fontColor,0.15f) )
						{
							...
							if ( _renderMode == BarGraphRenderMode.Lines )
							{
								...
								{
									if ( pointIndex > 0 )
										graph.DrawLine( linePen, linePoints[pointIndex-1], linePoints[pointIndex] );
								}
								...
								if (_displayBarData&&(i%_interval)==0)
								{
									...
									for ( int box = -1; box <= 1; ++box )
									{
										for ( int boy = -1; boy <= 1; ++boy )
										{
											...
											graph.DrawString(item.Value.ToString("#,###.##"), valFont, Brushes.White, rco, sfFormat);
										}
									}
